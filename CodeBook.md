...
## Instruction
1. Download the file from the website link and unzip the file by download.file() and unzip().
2. Read data from the targeted files by read.table().
3. Set the activity and subject ID for training and test data sets and merge them to create one data set by rbind().
4. Extract only the measurements on the mean and standard deviation for each measurement by grepl() and then subset.
5. Use descriptive activity names from *activity_labels.txt* to name the activities in the data set.
6. Label the data set with descriptive variable names by gsub().
7. Calculate the average of each variable for each activity and each subject by aggregate().
